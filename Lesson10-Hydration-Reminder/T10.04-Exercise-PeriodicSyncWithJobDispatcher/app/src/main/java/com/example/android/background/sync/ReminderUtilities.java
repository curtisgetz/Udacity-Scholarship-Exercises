...
public class ReminderUtilities {
    ...
    private static final String REMINDER_JOG_TAG = "hydration-reminder-tag";
    ...
    synchronized public static void scheduleChargingReminder(Context context){
        ...
        Job myJob = jobDispatcher.newJobBuilder().setService(WaterReminderFirebaseJobService.class)
    ...
    // Checkout https://github.com/firebase/firebase-jobdispatcher-android for an example
        ...
        //   (20) Use FirebaseJobDispatcher's newJobBuilder method to build a job which:
            //x - has WaterReminderFirebaseJobService as it's service
            //x - has the tag REMINDER_JOB_TAG
            // - only triggers if the device is charging
            //x - has the lifetime of the job as forever
            //x - has the job recur
            //x - occurs every 15 minutes with a window of 15 minutes. You can do this using a
            //x   setTrigger, passing in a Trigger.executionWindow
